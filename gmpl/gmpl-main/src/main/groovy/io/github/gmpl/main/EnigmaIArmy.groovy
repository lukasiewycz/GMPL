...
def etw = 'abcdefghijklmnopqrstuvwxyz'
...
def reflectors = [
        'A':      [wires: 'ejmzalyxvbwfcrquontspikhgd']
]

def reflectorAdjustable = false

def plugboardLimit = 10


def strInput  = 'wetterbericht'
def strOutput = 'jzmqnbaldjflj'
...
def zb = var name: 'zb', type: boolean, domain: ['a'..'z','a'..'z'] // plugboard variables
...
if(plugboardLimit==0){
        ...
        for(b0 in 'a'..'z') for(b1 in 'a'..'z') if(b0 != b1) {
                constraint compare(zb[b0,b1]-zb[b1,b0],'==',0)
        }
        for(b in 'a'..'z'){
                constraint compare(sum(zb[b,_]),'==',1)
                constraint compare(sum(zb[_,b]),'==',1)
        }

        for(l in 'a'..'z') for(b in 'a'..'z') for(t in 1..T) {
                def m = getOutput(etw, b)
                clause ~io[l,t] | ~zb[l,b] | is[0,m,t]
                clause io[l,t] | ~zb[l,b] | ~is[0,m,t]
                //constraint compare(io[l,t]-is[0,m,t],'==',0)
        }
        ...
        constraint compare(sum, '<=', plugboardLimit)
}
...
while(solve()){
        ...
        for(r in rotors.keySet()) if(disjunction(xs[r,_] as GLiteral[]) as boolean) {
                ...
                for(b0 in 'a'..'z') for(b1 in 'a'..'z') if(plugboardLimit>0 && b0 <= b1 && zb[b0,b1] as boolean) list << zb[b0,b1]
...
for (f in reflectors.keySet()){
        ...
        for(p in 'a'..'z') if(yp[p] as boolean) print "($p)"
}

if(plugboardLimit>0){
        print '\t\tplugboard: '
        for(b0 in 'a'..'z') for(b1 in 'a'..'z') if(b0 < b1){
                if(zb[b0,b1] as boolean) print b0+'-'+b1+' '
        }
}
println ''
